    ...
    namespace impl {
        ...
        class stream_ {
            ...
            friend stream_ filter_optional_(
                ...
                const std::function<
                    boost::optional<light_ptr>(const light_ptr&)>& f);
            ...
            std::function<void()>* listen_raw(
                ...
                std::function<void(const SODIUM_SHARED_PTR<impl::node>&,
                                   transaction_impl*, const light_ptr&)>*
                    handle,
            ...
            stream_ coalesce_(transaction_impl* trans,
                              const std::function<light_ptr(const light_ptr&,
                                                            const light_ptr&)>&
                                  combine) const;
            stream_ last_firing_only_(transaction_impl* trans) const;
            stream_ snapshot_(transaction_impl* trans, const cell_& beh,
                              const std::function<light_ptr(const light_ptr&,
                                                            const light_ptr&)>&
                                  combine) const;
            stream_ filter_(
                transaction_impl* trans,
                const std::function<bool(const light_ptr&)>& pred) const;
...
#define SODIUM_DETYPE_FUNCTION1(A, B, f)                      \
                ...
                new std::function<void(const SODIUM_SHARED_PTR<impl::node>&,
                    ...
                    [pState, f](const SODIUM_SHARED_PTR<impl::node>& target,
                        ...
                        SODIUM_TUPLE<B, S> outsSt =
                            f(*ptr.cast_ptr<A>(NULL), (*pState)());
                        const S& new_s = SODIUM_TUPLE_GET<1>(outsSt);
                        *pState = lazy<S>(new_s);
                        send(target, trans2,
                             light_ptr::create<B>(SODIUM_TUPLE_GET<0>(outsSt)));
                    }),
                false);
    ...
    template <typename A> class stream : protected impl::stream_ {
        ...
        std::function<void()> listen(
            ...
            std::function<void()>* pKill = listen_raw(
                trans1.impl(),
                SODIUM_SHARED_PTR<impl::node>(
                    new impl::node(SODIUM_IMPL_RANK_T_MAX)),
                new std::function<void(const SODIUM_SHARED_PTR<impl::node>&,
                                       impl::transaction_impl*,
                                       const light_ptr&)>(
                    [handle](const SODIUM_SHARED_PTR<impl::node>&,
                             impl::transaction_impl*, const light_ptr& ptr) {
         ...
         */
        stream<A> coalesce(
            const std::function<A(const A&, const A&)>& combine) const {
            transaction trans;
            stream<A> sa(coalesce_(
                trans.impl(),
                [combine](const light_ptr& a, const light_ptr& b) -> light_ptr {
                    return light_ptr::create<A>(
                        combine(*a.cast_ptr<A>(NULL), *b.cast_ptr<A>(NULL)));
                }));
            trans.close();
            return sa;
        }
    ...
    namespace impl {
        stream_ filter_optional_(
            transaction_impl* trans, const stream_& input,
            const std::function<boost::optional<light_ptr>(const light_ptr&)>&
                f);
    }
